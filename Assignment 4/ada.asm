...
MADD1: 		move $t0 $a3	# n
		...
		li $t2 0	# j
		li $t3 0	# k
		mul $s7 $t0 4		# Figures out how many bytes in a single row
		...
		rowFinal: 	addi $t1 $t1 1
				...
				colLoop:	beq $t2 $t0 rowFinal
						mul $s5 $s7 $t1		# Calculates overall row (bytes in row * current row num)
						mul $s6 $t2 4		# Calculates overall col in row (current col number * 4)
						add $t6 $s6 $s5		# C Location Offset (Start row + start col)
				...
				colFinal:	addi $t2 $t2 1
						...
						sumLoop:	beq $t3 $t0 colFinal			
								mul $t8 $t3 $s7 	# Calculate third loop row offset (k * bytes in row)
								mul $t7 $t3 4		# Calculate third loop col offset (k * byte block)
								add $t4 $s5 $t7		# A location Offset (Start row + k col)
								add $t5 $s6 $t8		# B Location Offset (Start col + k row)	
